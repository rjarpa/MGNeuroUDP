...
for i=1:3
    ...
    plot(t,M(i,:)');
    title("Participante: " + dtnames{participant_n});
    % DRL: aquí puedes agregar 'interpreter', 'none' para que no tome los
    % guiones bajos como instrucción de subíndice
...
%% 4
    ...
    plot(t,(epochs_Norm_mean')+(epochs_Norm_std'),'r')
    % se grafica la media  - desviacion standard
    plot(t,(epochs_Norm_mean')-(epochs_Norm_std'),'b')
    title("Participante: " + dtnames{participant_n});
    legend ("Mean Normalization - " + getfield(INFO,Norm2Use),...
...
% DRL: Bien
% DRL: En general, se grafica la media con una línea más gruesa, y las dos
% SD con el mismo color, para que no parezcan 3 señales 
...
% se ocupa ciclo para graficar la informacion de cada participante
    ...
    plot(t,M_mean(i,:)')
    % se grafica la media + desviacion standard
    plot(t,M_mean(i,:)'+M_std(i,:)','r')
    % se grafica la media - desviacion standard
    plot(t,M_mean(i,:)'-M_std(i,:)','b')
    title("Participante: " + dtnames{participant_n});
    xlabel('Time (s)');
    ylabel("Pupil Diameter " );
    legend('mean','+sd','-sd');
...
figure('Name','Figure 8');
   ...
   % se grafican ambas variables anteriores
    ...
    p4=plot(t,neg_sd,'ob','LineWidth', 2);
        ...
        % DRL: en este caso, no era necesario graficar pos_sd ni neg_sd,
        % DRL: dado que el patch se encarga de eso.

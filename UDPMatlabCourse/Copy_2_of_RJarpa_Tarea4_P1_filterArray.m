...
% Utilizando la funcion size para obtener las dimensiones de la matriz
    ...
    auxArraylostGSNickel    = lostArray(lostArray(:,1)==jj && )
    auxArraylostGSDollar    = lostArray(lostArray(:,1)==jj)
    auxArraylostNGSNickel   = lostArray(lostArray(:,1)==jj)
    auxArraylostNGSDollar   = lostArray(lostArray(:,1)==jj)
    
    
    return
    
    [rowsLost,colsLost] = size(auxLost)

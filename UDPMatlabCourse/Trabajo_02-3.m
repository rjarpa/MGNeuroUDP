...
for xx=1:numel(nArray)
    simu3=randi([1 val_posibles],nArray(xx),m);
    %se saca el promedio de cada muestra y luego la media de todas las
    %muestras
    prom3=mean(mean(simu3));
    prom3=round(prom3,1);
    promedios(xx)=prom3;
end 

%valores unicos de las medias
    value_unique_3=unique(promedios) ;
    p_3 = [];
    
   %ciclo para contar cuantos valores unicos existian en cada media.
    for ii=1:numel(value_unique_3)
        cuantos=sum(promedios==value_unique_3(ii));
        p_3(ii) = cuantos/m;
    end
    ...
    bar(value_unique_3, p_3,'DisplayName',"Dist de medias de n ="+nInicio+" hasta " +nFin );
    dim=[.15 .8 .2 0];

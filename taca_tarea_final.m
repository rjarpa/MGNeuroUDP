...
disp("codigo bloque human");
mdl_bloc1=fitlm(offer_part_b1,porce_part_b1)
tbl_bloc1=anova(mdl_bloc1)
%ttest_bloc1=ttest(offer_part_b1,porce_part_b1)
var_Estimate=mdl_bloc1.Coefficients.Estimate;

f1=figure('Name','Tipo de oferta bloque 1 - human');
hold on;
...
X1=[min(offer_part_b1) max(offer_part_b1)];
Y1= B0 + B1 .* X1 ;
plot(X1,Y1,'-r');
...
annotation('textbox',dim,'String',"Model Equation AR=" + B0+" + " + B1+ " * Offers ",'FitBoxToText','on','FontSize',14);
%dim=[.15 .6 .2 0];
...
simu_distrmuestral(mdl_bloc1,tbl_bloc1,offer_part_b1,B1 ,1000,'Bloque Human','B1','Probability')


%% codigo bloque computer 1


disp("codigo bloque Computer");

mdl_bloc2=fitlm(offer_part_b2,porce_part_b2)
tbl_bloc2=anova(mdl_bloc2)
var_Estimate=mdl_bloc2.Coefficients.Estimate;

f2=figure('Name','Tipo de oferta bloque 2 - Computer');;
hold on;
%scatter(porce_acepta_part_block1,promedio_offer_part_block1,'.r','jitter','on','jitterAmount',0.02);
...
dim=[.15 .8 .2 0];
%annotation('textbox',dim,'String',"Model Equation Volume=" + a_hat_2+" + " + b_hat_2+ " * Height + e(0,"+s_2+")",'FitBoxToText','on','FontSize',8);
annotation('textbox',dim,'String',"Model Equation AR=" + B0+" + " + B1+ " * Offer ",'FitBoxToText','on','FontSize',14);
...
simu_distrmuestral(mdl_bloc2,tbl_bloc2,offer_part_b2,B1 ,1000,'Bloque Computer','B1','Probability')
...
f3=figure('Name','Modelo Lineal y Bloque Human Computer ');
%hold on;
ax=axes('parent', f3, 'tickdir', 'out', 'xtick', [1 2], 'xticklabel', {'Human', 'Computer'}, 'fontsize', fsize);
hold(ax,'all');
%scatter(porce_acepta_part_block1,promedio_offer_part_block1,'.r','jitter','on','jitterAmount',0.02);
...
dim=[.15 .9 .2 0];
%annotation('textbox',dim,'String',"Model Equation Volume=" + a_hat_2+" + " + b_hat_2+ " * Height + e(0,"+s_2+")",'FitBoxToText','on','FontSize',8);
annotation('textbox',dim,'String',"Model Equation AR=" + B0+" + " + B1+ " * Block ",'FitBoxToText','on','FontSize',12,'Color','Black');
%dim=[.15 .8 .2 0];
%annotation('textbox',dim,'String',"Model Equation Volume=" + a_hat_2+" + " + b_hat_2+ " * Height + e(0,"+s_2+")",'FitBoxToText','on','FontSize',8);
...
%MANERA CORRECTA
simu_distrmuestral(LM,tbl_,type_offer_x, B1,1000,'Distribucion Muestral- Computer Human','B1','Probability')
...
f2_2=figure('Name','Barras Human vs Computer ');
...
disp('Modelo Lineal Simple Human')
...
porce_part_face_b1 = [];
group_face_b1 = [];
face_part_face_b1 = []; %utilizado para ver si las caras afectan
...
for i= 1:numel(unique(participant_id))
    for j= 1:numel(face_type)
        ...
        choice_y_block1_parxoffer=choice_y((block==1)&(participant_id==i)&(face==face_type(j)));
        
        cantidad_opciones=numel(choice_y_block1_parxoffer);
        suma_choice=sum(choice_y_block1_parxoffer);
        promedio_choice_partxface_block1=suma_choice/cantidad_opciones;
        
        id_part_face_b1(cont_arra) = i;
        %offer_part_face_b2(cont_arra) = offer_type(j);
        porce_part_face_b1(cont_arra) = promedio_choice_partxface_block1;
        group_face_b1(cont_arra)=2;
        face_part_face_b1(cont_arra) = face_type(j) ; 
...
porce_part_face_b1= transpose(porce_part_face_b1);
face_part_face_b1=transpose(face_part_face_b1);

% porcentaje de aceptacion de en el bloque de caras
T_face=table(porce_part_face_b1,face_part_face_b1);
LM_face=fitlm(T_face,'porce_part_face_b1~1+face_part_face_b1','ResponseVar','porce_part_face_b1',...
    'PredictorVars',{'face_part_face_b1'},'CategoricalVar',{'face_part_face_b1'})
...
%face angry
XG1=[min(face_part_face_b1) max(face_part_face_b1)];
YG1=B0 + B1.*XG1;
...
f4=figure('Name','Modelo Lineal Tipo de Expresiones Faciales');
%hold on;

ax=axes('parent', f4, 'tickdir', 'out', 'xtick', face_type, 'xticklabel', {'Negative','Neutral','Positive'}, 'fontsize', fsize);
hold(ax,'all');


scatter(face_part_face_b1(face_part_face_b1==-1),porce_part_face_b1(face_part_face_b1==-1),'or','jitter','on','jitterAmount',0.02);
scatter(face_part_face_b1(face_part_face_b1==0),porce_part_face_b1(face_part_face_b1==0),'ob','jitter','on','jitterAmount',0.02);
scatter(face_part_face_b1(face_part_face_b1==1),porce_part_face_b1(face_part_face_b1==1),'ok','jitter','on','jitterAmount',0.02);
% se grafica la ecuacion de la recta
%plot(XG1,YG1,'-b');
%plot(XG2,YG2,'-b');
%plot(XG2,YG2,'-k');

plot([-1.05 -0.95], [mean(porce_part_face_b1(face_part_face_b1==-1)) mean(porce_part_face_b1(face_part_face_b1==-1))],'-r')
plot([-0.05 0.05], [mean(porce_part_face_b1(face_part_face_b1==0)) mean(porce_part_face_b1(face_part_face_b1==0))], '-b')
plot([0.95 1.05], [mean(porce_part_face_b1(face_part_face_b1==1)) mean(porce_part_face_b1(face_part_face_b1==1))], '-k')

%se gregan etiquetas de los ejes
xlabel('Type of Faces');
ylabel('Acceptance Rate');

dim=[.15 .9 .2 0];
%annotation('textbox',dim,'String',"Model Equation Volume=" + a_hat_2+" + " + b_hat_2+ " * Height + e(0,"+s_2+")",'FitBoxToText','on','FontSize',8);
%annotation('textbox',dim,'String',"Model Equation AR=" + B0+" + " + B1+ " * Faces ",'FitBoxToText','on','FontSize',12,'Color','Black');
annotation('textbox',dim,'String',"Model Equation R-square=" + LM_face.Rsquared.Ordinary,'FitBoxToText','on','FontSize',12,'Color','Black');
...
for i= 1:numel(unique(participant_id))
    for j= 1:numel(face_type)
        ...
        choice_y_block1_parxoffer=choice_y((block==1)&(participant_id==i)&(face==face_type(j))&(offer_x==offer_type(x)));
        ...
        id_part_face_b1(cont_arra) = i;
        offer_part_face_b1(cont_arra) = offer_type(x);
        porce_part_face_b1(cont_arra) = promedio_choice_partxface_block1;
        group_face_b1(cont_arra)=2;
        face_part_face_b1(cont_arra) = face_type(j) ; 
...
%resolucion para nan
T_face_1_nan=table(id_part_face_b1,porce_part_face_b1,face_part_face_b1,offer_part_face_b1);

% se evalua el % de aceptacion cuando se muestran rostros por grupos
id_part_face_b1= transpose(id_part_face_b1(~isnan(porce_part_face_b1)));
face_part_face_b1=transpose(face_part_face_b1(~isnan(porce_part_face_b1)));
%face_part_face_b1=face_part_face_b1+1
offer_part_face_b1=transpose(offer_part_face_b1(~isnan(porce_part_face_b1)));
porce_part_face_b1= transpose(porce_part_face_b1(~isnan(porce_part_face_b1)));

% porcentaje de aceptacion de en el bloque de caras
T_face_2=table(id_part_face_b1,porce_part_face_b1,face_part_face_b1,offer_part_face_b1);
% independiente
LM_face_2=fitlm(T_face_2,'porce_part_face_b1~1+offer_part_face_b1+face_part_face_b1','ResponseVar','porce_part_face_b1',...
    'PredictorVars',{'face_part_face_b1','offer_part_face_b1'},'CategoricalVar',{'face_part_face_b1'})
...
f5_1=figure('Name','Modelo Lineal valores de caras categorico mas  oferta');
hold on;


face_angry_x_group=offer_part_face_b1(face_part_face_b1==-1);
angry_min=min(face_angry_x_group);
angry_max=max(face_angry_x_group);

face_neutral_x_group=offer_part_face_b1(face_part_face_b1==0);
neutral_min=min(face_neutral_x_group);
neutral_max=max(face_neutral_x_group);

face_happy_x_group=offer_part_face_b1(face_part_face_b1==1);
happy_min=min(face_happy_x_group);
happy_max=max(face_happy_x_group);


XG3=[min(happy_min) max(happy_max)];
YG3= B0 + (B1) .* XG3 + B2 + B3 .* (XG3) ;

% face  neutral 
XG2=[min(neutral_min) max(neutral_max)];
YG2=B0 + (B1) .* XG2 + B2   ;

%face angry
XG1=[min(angry_min) max(angry_max)];
YG1=B0 + B1.*XG1;


porce_angry_y_group=porce_part_face_b1(face_part_face_b1==-1);
porce_neutral_y_group=porce_part_face_b1(face_part_face_b1==0);
porce_happy_y_group=porce_part_face_b1(face_part_face_b1==1);
...
legend('% negative','% neutral','% positive','LM negative ','LM neutral', 'LM positive');
...
%face happy
XG3=[min(offer_part_face_b1) max(offer_part_face_b1)];
%YG3= B0 + (B1) .* XG3 + B2 + B3 .* (XG3) + B4 + B5 .*(XG3);
YG3= B0 + (B1) .* XG3 + B2 .* (XG3) + B3 .* (XG3) + B4  .*(XG3) + B5 .*(XG3);

% face  neutral 
XG2=[min(offer_part_face_b1) max(offer_part_face_b1)];
%YG2=B0 + (B1) .* XG2 + B2  +B3 .* (XG2)  ;
YG2=B0 + (B1) .* XG2 + B2 .* (XG2) +B3 .* (XG2)  ;

%face angry
XG1=[min(offer_part_face_b1) max(offer_part_face_b1)];
YG1=B0 + B1.*XG1;
...
%% Oferta y tipos de rostos Modelo Simple Positive 


% porcentaje de aceptacion de en el bloque de caras
T_face_positive=table(porce_part_face_b1(face_part_face_b1==1),...
    offer_part_face_b1(face_part_face_b1==1),...
    'VariableNames',{'porce_part_face_b1','offer_part_face_b1'});
% independiente
LM_face_positive=fitlm(T_face_positive,'porce_part_face_b1~1+offer_part_face_b1','ResponseVar','porce_part_face_b1',...
    'PredictorVars',{'offer_part_face_b1'})
tbl_face_happy=anova(LM_face_positive)

var_Estimate_face_2=LM_face_positive.Coefficients.Estimate;
B0=var_Estimate_face_2(1);
B1=var_Estimate_face_2(2);

%face angry
XG1=[min(porce_part_face_b1(face_part_face_b1==1)) max(porce_part_face_b1(face_part_face_b1==1))];
YG1=B0 + B1.*XG1;

f10=figure('Name','Modelo Lineal Rostros Positivos');
%hold on;
ax=axes('parent', f10, 'tickdir', 'out', 'xtick', offer_type, 'xticklabel', offer_type, 'fontsize', fsize);
hold(ax,'all');


scatter(offer_part_face_b1(face_part_face_b1==1),porce_part_face_b1(face_part_face_b1==1),'og','jitter','on','jitterAmount',0.02);
% se grafica la ecuacion de la recta
plot(XG1,YG1,'-g');
%plot(XG2,YG2,'-m');
%plot(XG3,YG3,'-g');
...
dim=[.15 .9 .2 0];
%annotation('textbox',dim,'String',"Model Equation Volume=" + a_hat_2+" + " + b_hat_2+ " * Height + e(0,"+s_2+")",'FitBoxToText','on','FontSize',8);
annotation('textbox',dim,'String',"Model Equation AR=" + B0+" + " + B1+ " * Offer ",'FitBoxToText','on','FontSize',12,'Color','Black');
...
% porcentaje de aceptacion de en el bloque de caras
T_face_neutral=table(porce_part_face_b1(face_part_face_b1==0)...
    ,offer_part_face_b1(face_part_face_b1==0),...
    'VariableNames',{'porce_part_face_b1','offer_part_face_b1'});
% independiente
LM_face_neutral=fitlm(T_face_neutral,'porce_part_face_b1~1+offer_part_face_b1','ResponseVar','porce_part_face_b1',...
    'PredictorVars',{'offer_part_face_b1'})
...
%face angry
XG1=[min(porce_part_face_b1(face_part_face_b1==0)) max(porce_part_face_b1(face_part_face_b1==0))];
YG1=B0 + B1.*XG1;

f10=figure('Name','Modelo Lineal Rostros Neutros');
hold on;
scatter(offer_part_face_b1(face_part_face_b1==0),porce_part_face_b1(face_part_face_b1==0),'om','jitter','on','jitterAmount',0.02);
% se grafica la ecuacion de la recta
plot(XG1,YG1,'-m');
%plot(XG2,YG2,'-m');
%plot(XG3,YG3,'-g');
...
% porcentaje de aceptacion de en el bloque de caras
T_face_negative=table(porce_part_face_b1(face_part_face_b1==-1),...
    offer_part_face_b1(face_part_face_b1==-1),...
    'VariableNames',{'porce_part_face_b1','offer_part_face_b1'});
% independiente
LM_face_negative=fitlm(T_face_negative,'porce_part_face_b1~1+offer_part_face_b1','ResponseVar','porce_part_face_b1',...
    'PredictorVars',{'offer_part_face_b1'})
...
f10=figure('Name','Modelo Lineal Rostros Negativos');
hold on;
scatter(offer_part_face_b1(face_part_face_b1==-1),porce_part_face_b1(face_part_face_b1==-1),'ob','jitter','on','jitterAmount',0.02);
% se grafica la ecuacion de la recta
plot(XG1,YG1,'-b');
%plot(XG2,YG2,'-m');
%plot(XG3,YG3,'-g');
...
Permuta=test_permuta(porce_part_b1, porce_part_b2,150000);
...
chart_histo2(Permuta,'Grafica de Distribucion de Medias de Permutaciones','Diferencia de Medias',D)
...
function chartBool=chart_histo2(x_values,var_figure_name,var_x_label,pendiente)
...
dim=[.15 .7 .2 0];
annotation('textbox',dim,'String',"Orignal Media muestra=" + pendiente,'FitBoxToText','on','FontSize',14);

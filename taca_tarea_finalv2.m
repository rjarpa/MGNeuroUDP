...
B0=var_Estimate(1)
B1=var_Estimate(2)
X2=[min(offer_part_b2) max(porce_part_b2)];
Y2= B0 + B1 .* X1 ;
plot(X2,Y2,'-b');
%se gregan etiquetas de los ejes
xlabel('Offer');
...
YG2=B0 + (B1) .* XG2 + B2;
XG1=[min(type_offer_x) max(type_offer_x)];
YG1=B0 + B1 .*XG1;


f3=figure('Name','Modelo Lineal Multiple Oferta y Bloque Con_Sin Caras ');
hold on;
%scatter(porce_acepta_part_block1,promedio_offer_part_block1,'.r','jitter','on','jitterAmount',0.02);
scatter(offer_part_b1,porce_part_b1,'.r','jitter','on','jitterAmount',0.02);
scatter(offer_part_b2,porce_part_b2,'.b','jitter','on','jitterAmount',0.02);
...
dim=[.15 .9 .2 0];
%annotation('textbox',dim,'String',"Model Equation Volume=" + a_hat_2+" + " + b_hat_2+ " * Height + e(0,"+s_2+")",'FitBoxToText','on','FontSize',8);
annotation('textbox',dim,'String',"Model Equation Volume=" + B0+" + " + B1+ " * Offer ",'FitBoxToText','on','FontSize',12,'Color','Red');
dim=[.15 .8 .2 0];
%annotation('textbox',dim,'String',"Model Equation Volume=" + a_hat_2+" + " + b_hat_2+ " * Height + e(0,"+s_2+")",'FitBoxToText','on','FontSize',8);
...
print(f3, 'Modelo Lineal Multiple Oferta y Bloque Con_Sin Caras', '-dpng', '-r350');


%simulacion de grupo de oferta y bloque REVISAR SI SE ESTA EVALUANDO DE
%MANERA CORRECTA
simu_distrmuestral(LM,tbl_,type_offer_x, B1,1000,'Tipo de Oferta - Categorica','Diff of Means','Probability')



%plot(rt_y,choice_y,'ok');
legend('Con Caras ','Sin Caras')
...
hold on;
scatter(face_part_face_b2,porce_part_face_b2,'ob','jitter','on','jitterAmount',0.02);
% se grafica la ecuacion de la recta
plot(XG1,YG1,'-r');
%plot(XG2,YG2,'-b');
%se gregan etiquetas de los ejes
...
var_Estimate_face=LM_face_2.Coefficients.Estimate;
...
YG3= B0 + (B1) .* XG2 + B2 + B3 .* (XG3) + B4 + B5 .*(XG3);

% face  neutral 
XG2=[min(offer_part_face_b2) max(offer_part_face_b2)];
YG2=B0 + (B1) .* XG2 + B2 +B3 .* (XG3)  ;
